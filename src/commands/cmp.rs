...
pub fn command(args: &[&str]) {
    ...
    if diff_bytes == 0 && total1 == total2 {
        ...
        let len_diff = if total1 > total2 { total1 - total2 } else { total2 - total1 };
        shell_println!("Files differ in length by %u bytes\nAdditionally found %u differing bytes", len_diff, diff_bytes);
    }
    unsafe {
        fsutil::close(fd1);
        fsutil::close(fd2);
